...
";
    ...
    fn parses_cache_rates_args() {
        let args = parse_args_from(vec!["2024".to_owned()], super::USAGE).expect("args");
        assert_eq!(
            args,
            CacheRatesArgs {
                year: 2024,
    ...
    fn parses_cache_rates_vwap_flag() {
        let args = parse_args_from(vec!["--vwap".to_owned(), "2024".to_owned()], super::USAGE)
            .expect("args");
        assert_eq!(
            args,
            CacheRatesArgs {
    ...
    fn parses_cache_rates_until_day_inclusive() {
        let args = parse_args_from(
            vec!["--until=2026-06-30".to_owned(), "--vwap".to_owned()],
            super::USAGE,
        )
        .expect("args");
    ...
    fn rejects_cache_rates_until_for_conflicting_year() {
        ...
        assert!(
            err.to_string()
                .contains("does not match requested year 2026")
        );
    }

    #[test]
    fn rejects_extra_cache_rates_args() {
        let err = parse_args_from(vec!["2024".to_owned(), "extra".to_owned()], super::USAGE)
            .expect_err("should fail");
